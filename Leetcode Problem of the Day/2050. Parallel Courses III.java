...
completed before course nextCoursej (prerequisite relationship). Furthermore, you are given a 0-indexed integer array time 
...
class Solution {
    public int minimumTime(int n, int[][] relations, int[] time) {        
        ...
        Queue<Integer> startCourse = new LinkedList<>();
        int[] dp = new int[n];
        ...
        while(!startCourse.isEmpty()){
            int curCourse = startCourse.poll();
            
            for(int nextCourse : graph[curCourse]){
                dp[nextCourse]  = Math.max(dp[nextCourse], time[nextCourse] + dp[curCourse]);
                inDegree[nextCourse]--;
                if(inDegree[nextCourse]==0) startCourse.add(nextCourse);
            }
            result = Math.max(result, dp[curCourse]);
        }
