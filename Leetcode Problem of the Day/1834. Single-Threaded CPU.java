...
where tasks[i] = [enqueueTimei, processingTimei] means that the ith task will be available to
    ...
    public int[] getOrder(int[][] tasks) {
        ...
        */
        ...
        while (taskIndex < tasks.length || !nextTask.isEmpty()) {
            if (nextTask.isEmpty() && currTime < sortedTasks[taskIndex][1]) {
                currTime = sortedTasks[taskIndex][1];
            }
            
            while (taskIndex < tasks.length && currTime >= sortedTasks[taskIndex][1]) { 
                nextTask.add(sortedTasks[taskIndex]);
                ++taskIndex;
            }
            
            int processTime = nextTask.peek()[2];
            int index = nextTask.peek()[0];
            nextTask.remove();
