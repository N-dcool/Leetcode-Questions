...
class Solution {
    ...
    public boolean helper(int i){
        ...
        if(i-1>= 0 && nums[i]==nums[i-1]) ans |= helper(i-2);
        if(i-2>=0 && nums[i]==nums[i-1] && nums[i-1]==nums[i-2]) ans |= helper(i-3);
        if(i-2>=0 && nums[i]==nums[i-1]+1 && nums[i-1]+1==nums[i-2]+2) ans |= helper(i-3);
