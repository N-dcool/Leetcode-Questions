...
class Solution {
    ...
    public boolean helper(int stone, int k){
        if(stone>=lastStone)
            return true;
        int idx = map.get(stone);
        ...
        for(int jump=k-1; jump<=k+1; jump++){
            ...
            if(map.containsKey(nextStone) && nextStone > stone)
                ans |= helper(nextStone, jump);
        }
