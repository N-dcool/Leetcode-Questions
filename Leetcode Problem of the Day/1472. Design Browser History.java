...
class BrowserHistory {
    ...
    public BrowserHistory(String homepage) {
        ...
        visitedLinks.add(homepage);
        curTab = 0;
    }
    
    public void visit(String url) {
        if(curTab == visitedLinks.size()-1)
            visitedLinks.add(url);
        else{
            int n = visitedLinks.size() - curTab - 1;
            // visitedLinks.subList(curTab+1, visitedLinks.size()-1).clear();
            while(n-->0){
    ...
    public String back(int steps) {
        int max = Math.max(0, curTab-steps);
        curTab = max;
        return visitedLinks.get(curTab);
        
    }
    
    public String forward(int steps) {
        int min = Math.min(visitedLinks.size()-1, curTab+steps);
        curTab = min;
        return visitedLinks.get(min);
    }
}
