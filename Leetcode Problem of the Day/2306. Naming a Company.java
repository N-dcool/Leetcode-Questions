...
//                Only 89 testcases Passed :

class Solution {
    public long distinctNames(String[] ideas) {
        HashSet<String> setIdeas = new HashSet<>();
        int n = ideas.length;
        ...
        for(int i=0; i<n; i++){
            for(int j=i+1; j<n; j++){
                String idea1 = ideas[i].charAt(0) + ideas[j].substring(1);
                String idea2 = ideas[j].charAt(0) + ideas[i].substring(1);
                
                if(!(setIdeas.contains(idea1) || setIdeas.contains(idea2))){
                    if(!companyName.contains(idea1 + idea2)){
                        count++;
                        companyName.add(idea1 + idea2);
                    }
                    if(!companyName.contains(idea2 + idea1)){
                        count++;
                        companyName.add(idea2 + idea1);
                    }
